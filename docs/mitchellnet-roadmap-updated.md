...
Last updated: 10 July 2026 session. The Experiment 2 workflow revisit scoped at the end of the 8 July session (DMM R1/C1 measurement timing, side-by-side real/virtual oscilloscope GUI, AI-generated sim-vs-measured analysis) is now fully built and live-hardware-verified across two sessions: RC-Experiments PR #15 (9 July, Goal 1) and PRs #16-#21 + bench-instrument-service PR #43 (10 July, Goals 2-3, screenshot support, and doc updates). A real bug (Windows-only UTF-8 encoding failure in the AI Analysis file-write path) was found and fixed during live verification -- see the new § 4.10. Three documentation deliverables also shipped 10 July: existing markdown docs updated, formal RC-Experiments_BRD.docx/HLA.docx created (mirroring BIS's existing BRD/HLA), and experiment-2-series-rc/docs/experiment2Guide.md written -- a from-scratch walkthrough for teachers/students, later extended with a "the other scenarios" section (RC-Experiments PR #22). See the "Current state as of end of 10 July 2026 session" block at the end of this document for full detail. Andrew is putting RC-Experiments work on hold after this session.

⚠️ READ THIS FIRST NEXT SESSION: Start with the "Current state as of end of 10 July 2026 session" block at the end of this document. First action item (per § 1, now TEN+ sessions overdue and explicitly deferred again this session): rotate the BIS API key. RC-Experiments Experiment 2's workflow revisit is now COMPLETE and hardware-verified -- next RC-Experiments work (whenever resumed) is Experiment 1 (Bias-T), not started. § 4.7 / § 4.8 / § 4.9 / § 4.10 below are now historical record of resolved bugs -- still worth reading for diagnostic methodology and lessons, but all are closed.

Completed
    • ✅ 10 July 2026 session -- Experiment 2 workflow revisit Goals 2-3 built, entire three-goal revisit hardware-verified live, and all documentation deliverables shipped. RC-Experiments PR #16 (real oscilloscope screenshot capture + scope-styled simulation render), PR #17 (fixed a test-pollution bug that had let a fake screenshot get committed), PR #18 (--gui flag, new gui.py, Tkinter side-by-side comparison window; Pillow dependency added), PR #19 (on-demand "AI Analysis" button, new ai_analysis.py, Claude API with its own dedicated system prompt, mirroring the recipes app's client pattern; anthropic==0.49.0 pinned to match), PR #20 (real bug found live: AI Analysis file-write used Windows' default cp1252 encoding and crashed on a Unicode character in Claude's response, leaving the GUI stuck on "Analyzing..." forever even though the API call had already succeeded -- fixed with explicit encoding="utf-8" plus a try/except(OSError, UnicodeError) that never loses an already-returned result; see § 4.10), PR #21 (README.md, docs/architecture.md, docs/testing.md, and experiment-2-series-rc/README.md all updated for the new DMM flow, screenshot/GUI, and AI Analysis features), PR #22 (RC-Experiments_BRD.docx and RC-Experiments_HLA.docx added, mirroring bench-instrument-service's existing BRD/HLA structure; experiment-2-series-rc/docs/experiment2Guide.md written -- a from-scratch, beginner-audience walkthrough covering circuit build, bench setup, every CLI flag, and result interpretation, later extended with a "the other scenarios" section explaining how --scenario 1/2/3 map to different half-period/tau ratios). bench-instrument-service PR #43 (9 July, prerequisite for PR #16) added screenshot() to bench_client.py, wrapping the existing but previously-unused POST /v1/oscilloscope/screenshot endpoint. Entire flow (--remeasure-components, --bench, --gui, AI Analysis) live-hardware-verified end-to-end on the Windows/Parallels box, including finding and fixing the encoding bug live. BIS API key rotation explicitly deferred again this session at Andrew's request -- see § 1. Full detail in the "Current state as of end of 10 July 2026 session" block at the end of this document. Andrew is putting RC-Experiments on hold after this session.
    • ✅ 9 July 2026 session -- Experiment 2 workflow revisit Goal 1 built: RC-Experiments PR #15 added the interactive per-component DMM remeasure flow (resolve_component_value(), resolve_all_component_values()), replacing the old standalone-only measure_and_cache_component_values(). --remeasure-components now works standalone (update cache, exit) or combined with --sim-only/--bench (resolve interactively, then continue into the run), offering Enter-manually / Measure-via-DMM / Keep-current (default, only if cached) for each of C1 then R1. Two real bugs found and fixed during the build (not caught by the passing test suite, found via code review): a session token was being re-acquired on every DMM measurement instead of cached once across both components; the original circuit-reassembly reminder was silently dropped in the rewrite and had to be restored (fires whenever a component was actually put under the DMM, regardless of whether the reading was kept). 16 new tests (test_component_values.py) plus 10 tests (test_resolve_all_component_values.py, across the initial PR and its fix-up).
...
⚠️ BIS API key rotation is now TEN+ sessions overdue (pasted in plaintext 4 July, 5 July daytime, 5 July evening, 5 July late-evening, and again 6 July late-night — this exact key value has been visible in chat repeatedly; explicitly deferred again, by name, on 9-10 July: "lets not worry about the key rotation" in favour of the workflow-revisit build and hardware test). This should be treated as the literal first action item next session — it is cheap, safe, well-understood, and has now been deferred through an entire multi-session feature build purely because other work kept taking priority. Do not defer it again.
...
Feature/Build Work
    ...
    • Item 21 (RC-Experiments, renamed from RC-Circuit) — Experiment 2 (Series RC) is now fully complete and hardware-verified: § 4.7 (bench-capture corruption) and § 4.9 (time-domain alignment) resolved 7-8 July; the three-goal workflow revisit (DMM measurement timing, side-by-side real/virtual GUI, AI-generated analysis) built and hardware-verified 9-10 July, plus BRD/HLA and a full teaching-guide written 10 July — see § 4.10 and § 9. Experiment 1 (Bias-T) not started. Andrew is putting this item on hold after 10 July.
...
Found while building RC-Experiments' shared/src/bis_client.py and experiment-2-series-rc/src/run_experiment.py against the real bench_client.py and app/models/multimeter.py. All items below fixed in bench-instrument-service PRs #21–#23 (5 July 2026):
    ...
    4.10 ✅ RESOLVED 10 July 2026 — RC-Experiments AI Analysis Windows UTF-8 encoding bug, found live during hardware verification of the Experiment 2 workflow revisit

    SYMPTOM: on the Windows/Parallels box, clicking the "AI Analysis" button in the new --gui comparison window (RC-Experiments PR #19) appeared to hang indefinitely on "Analyzing..." — Andrew reported waiting several minutes with no response, expecting 10-20 seconds. This looked like a network/timeout issue at first glance.

    ROOT CAUSE: gui.py's on_analyze_click() calls ai_analysis.analyze_comparison() (which succeeded — the Claude API call completed and returned a real result), then writes that result to results/RC_Experiment2_S<n>_ai_analysis.md via open(analysis_path, "w") with no explicit encoding. On Windows, open() with no encoding= defaults to the system's active code page (cp1252 in this environment), not UTF-8. Claude's response text commonly includes Unicode characters (e.g. "~=" rendered as a proper approximately-equal sign), which cp1252 cannot encode — raising UnicodeEncodeError from inside f.write(), BEFORE the Tkinter text widget was updated to show the result. The uncaught exception silently killed the callback mid-execution, leaving the widget frozen on "Analyzing..." forever even though a complete, correct analysis was sitting in memory the whole time. Confirmed via the actual Windows terminal traceback, which Andrew pasted unprompted — that traceback was the key diagnostic; without it this would likely have been mis-investigated as a network problem.

    FIX (RC-Experiments PR #20): open(analysis_path, "w", encoding="utf-8") — explicit, not left to the platform default — wrapped in try/except (OSError, UnicodeError), appending a warning note to the in-memory result string on failure rather than losing it, with the widget update happening unconditionally afterward regardless of whether the file write succeeded. UnicodeEncodeError is a ValueError/UnicodeError subclass, not an OSError, so catching only OSError (the first instinct) would have missed exactly this failure — both exception types are caught. Live-reverified on the same Windows/Parallels box after the fix: AI Analysis now completes and displays within the expected ~10-20 seconds.

    LESSON: open()'s default encoding is platform-dependent (cp1252 on Windows, typically UTF-8 on macOS/Linux) — this is exactly why this bug only reproduced on the Windows/Parallels run environment and never showed up in the (Mac-only, mocked) pytest suite. Any file write containing text that didn't originate as pure ASCII (LLM output, in particular) needs an explicit encoding="utf-8", every time, regardless of what platform the code happens to have been tested on. See also § 7.11.
...
7.11 Lessons Learned — Cross-Platform File Encoding (10 July 2026)
    • Python's open(path, "w") with no encoding= uses the platform's default locale encoding, not UTF-8 — cp1252 on Windows, typically UTF-8 on macOS/Linux. Any code that writes text which isn't guaranteed pure ASCII (LLM-generated output is the common case in this project) must pass encoding="utf-8" explicitly, every time, regardless of which platform the code was developed and tested on.
    • This class of bug is invisible to a mocked test suite that only ever runs on macOS/Linux — it can only be caught by actually running the code on the target platform (here, Windows/Parallels). This is the same underlying lesson as § 7.7/§ 7.9 (passing tests prove the code does what it was written to do, not that it's correct under real conditions) applied to a new failure mode: cross-platform default behavior, not a hardware/timing issue.
    • UnicodeEncodeError is a subclass of ValueError/UnicodeError, not OSError — a bare except OSError around a file write (the natural first instinct, covering disk-full/permission-denied/path issues) will NOT catch an encoding failure. Catch both explicitly if a write's failure mode is unknown.
    • When a UI action appears to hang with no error visible in the GUI itself, get the actual underlying terminal/console traceback before guessing at network/timeout causes — in this case the real traceback (which Andrew supplied unprompted) immediately identified the true cause as a file-write exception raised after a successful API call, not a hung network request.
...
Infrastructure → Hosted Services
    ...
    • **`--bench` run has still never completed successfully end-to-end — six-plus attempts, all failing identically at the channel-2 capture step. See § 4.7 for the full diagnostic trail and next-session plan.** (Historical — resolved 7 July, see § 4.9.)
    • `experiment-2-series-rc/README.md` (the experiment-specific one, distinct from the repo-root README.md above) -- written 8 July 2026 (RC-Experiments PR #13); rewritten again 10 July 2026 (PR #21) to cover the workflow-revisit features below.
    • Workflow revisit (9-10 July 2026, PRs #15-#22) -- three goals from the 8 July planning discussion, all built and live-hardware-verified:
        1. Interactive DMM remeasure flow (PR #15, 9 July) -- `--remeasure-components` now works standalone or combined with a run, offering manual entry / DMM measurement / keep-current per component (C1 then R1), with a single shared BIS session across both and the circuit-reassembly reminder restored.
        2. Real oscilloscope screenshot + scope-styled simulation render, shown side by side in a new `--gui` Tkinter window (PRs #16, #18, plus bench-instrument-service PR #43 for the underlying `screenshot()` client method).
        3. On-demand "AI Analysis" button in that same window (PR #19), calling the Claude API with a dedicated RC-theory system prompt (own `ANTHROPIC_API_KEY`, not shared with the recipes app's integration).
        A real bug was found and fixed during live verification -- a Windows-only UTF-8 encoding failure in the AI Analysis file-write path (PR #20) -- see § 4.10. `docs/architecture.md`, `docs/testing.md`, and both README.md files updated to match (PR #21).
    • Formal documentation (10 July 2026, PR #22): `docs/RC-Experiments_BRD.docx` and `docs/RC-Experiments_HLA.docx` created, mirroring bench-instrument-service's existing BRD/HLA structure. `experiment-2-series-rc/docs/experiment2Guide.md` written -- a from-scratch walkthrough assuming no prior electronics/tooling background, covering circuit build, bench gear setup, every CLI flag through `--gui`/AI Analysis, and result interpretation -- later extended with a section explaining how the three `--scenario` values map to different half-period/tau ratios, and how a class could reach the same regimes by changing R1/C1 instead of frequency.

    Experiment 1 — Bias-T — status
    • Not yet started.

    Status: 🟢 Experiment 2 COMPLETE as of 10 July 2026 -- simulation, live bench capture, comparison engine (§ 4.7/§ 4.9), the full three-goal workflow revisit (DMM timing, side-by-side GUI, AI analysis -- § 4.10), and all documentation (markdown docs, BRD/HLA, experiment2Guide.md) are built, hardware-verified, and shipped. Andrew is putting RC-Experiments on hold after this session -- whenever it resumes, the next work is Experiment 1 (Bias-T), not started. See the "Current state as of end of 10 July 2026 session" block at the end of this document.
...
Current state as of end of 10 July 2026 session (supersedes the 8 July summary above for the Experiment 2 workflow revisit specifically -- the 8 July snapshot is left unedited above as accurate historical record of that point in time):

COMPLETED THIS SESSION (9-10 July, spanning two session boundaries -- logged together here since the roadmap wasn't updated in between):
    * RC-Experiments PR #15 (9 July) -- Goal 1: interactive per-component DMM remeasure flow. resolve_component_value()/resolve_all_component_values() replace the old standalone-only measure_and_cache_component_values(). --remeasure-components now works standalone or combined with --sim-only/--bench. Two real bugs found via code review (not caught by tests) and fixed: session token was being re-acquired per measurement instead of cached once; the circuit-reassembly reminder was silently dropped in the rewrite and had to be restored. 16 + 10 new tests.
    * bench-instrument-service PR #43 (9 July) -- screenshot() added to bench_client.py, wrapping the existing but previously client-unused POST /v1/oscilloscope/screenshot endpoint with binary response support. Prerequisite for RC-Experiments PR #16.
    * RC-Experiments PR #16 (10 July) -- Goal 2 part 1: real oscilloscope screenshot captured every --bench run (via the inherited BenchClient.screenshot(), no new wrapper code needed in this repo) and a scope-styled render of the simulated waveform (dark background, CH1/CH2-style trace colors) for visual pairing.
    * RC-Experiments PR #17 (10 July) -- fixed a pre-existing test (not one of tonight's new ones) that wasn't isolated to tmp_path and had let a 52-byte fake screenshot get committed into results/ during PR #16's promote. Caught via noticing the stray "create mode" line in the aaGitPromote output.
    * RC-Experiments PR #18 (10 July) -- Goal 2 part 2: --gui flag, new gui.py, Tkinter window showing the real screenshot and styled sim plot side by side plus the residual summary. Pillow added as a dependency (stdlib tkinter.PhotoImage doesn't handle BMP reliably).
    * RC-Experiments PR #19 (10 July) -- Goal 3: on-demand "AI Analysis" button in the same window. New ai_analysis.py mirrors the recipes app's Claude API integration pattern (lazy singleton client, own ANTHROPIC_API_KEY) but with its own dedicated system prompt, per the project's standing rule against sharing Claude API call functions across services. anthropic==0.49.0 pinned to match the recipes app's exact version.
    * RC-Experiments PR #20 (10 July) -- real bug found live during hardware verification and fixed: AI Analysis's file write used Windows' default cp1252 encoding, crashing on a Unicode character in Claude's response and leaving the GUI stuck on "Analyzing..." forever despite the API call having already succeeded. Fixed with explicit encoding="utf-8" plus a try/except (OSError, UnicodeError) that never loses an already-returned result. Full root cause and lesson in the new § 4.10 and § 7.11.
    * RC-Experiments PR #21 (10 July) -- README.md, docs/architecture.md, docs/testing.md, and experiment-2-series-rc/README.md all updated for the DMM flow, screenshot/GUI, and AI Analysis features, plus the 10 July hardware-verified state and the encoding bug.
    * Live end-to-end hardware verification (10 July, Windows/Parallels box): --remeasure-components, --scenario 2 --bench --gui, and the AI Analysis button all confirmed working together in one run, including finding and fixing the encoding bug live -- exactly the kind of thing a green mocked test suite cannot catch (same lesson as § 4.7/§ 4.9, new failure mode).
    * RC-Experiments PR #22 (10 July) -- formal documentation: docs/RC-Experiments_BRD.docx and docs/RC-Experiments_HLA.docx (mirroring bench-instrument-service's existing BRD/HLA structure and style), and experiment-2-series-rc/docs/experiment2Guide.md -- a from-scratch walkthrough for a teacher/student audience with no assumed background, covering circuit build, bench gear setup, software setup, every CLI flag through --gui/AI Analysis, and how to read and discuss the results. Extended in the same PR with a section explaining how the three built-in --scenario values (0.2 Hz / 1 Hz / 10 Hz) map to different half-period-to-tau ratios, and how the same three regimes could alternatively be reached by changing R1/C1 instead of frequency.
    * BIS API key rotation -- explicitly deferred again this session at Andrew's request ("lets not worry about the key rotation"), in favour of the hardware test and documentation work. Now TEN+ sessions overdue. Still the standing first-action-item for next session per § 1.
    * Andrew is putting RC-Experiments work on hold after this session.

EXPERIMENT 2 -- FINAL STATE (all three original workflow-revisit goals, complete):
    1. Component measurement timing -- interactive, integrated into the normal run flow, not a disconnected standalone-only step.
    2. Side-by-side real/virtual oscilloscope GUI -- real screenshot next to a scope-styled simulation render, plus the residual summary.
    3. AI-generated analysis -- on-demand, dedicated system prompt, explains the comparison in terms of RC theory and speculates (clearly flagged as speculation) on plausible real-world causes of any differences.
    All three, plus the underlying simulation/bench/comparison pipeline (§ 4.7, § 4.9), are hardware-verified working together in a single --scenario 2 --bench --gui run.

ACTIVE PROJECTS / NEXT SESSION OPTIONS, IN ORDER (whenever RC-Experiments work resumes):
    1. FIRST -- rotate the BIS API key. TEN+ sessions overdue now. Cheap, safe, unrelated to anything else. Do this before any other work in any repo.
    2. Item 21 -- RC-Experiments, Experiment 1 (Bias-T): not started. Expected to reuse the simulate/measure/compare/display/explain pattern established by Experiment 2, per docs/architecture.md and the RC-Experiments HLA § 3.3.
    3. BIS -- audit the other three instrument drivers (multimeter, power supply, signal generator) for the same driver-level SCPI wire-format test gap and live-hardware test tier that oscilloscope now has. See § 4.6.
    4. BIS -- audit request/response model consistency across all four instrument routers, per § 4.6.
    5. RC-Experiments -- BIS_REPO_PATH/BIS_API_KEY/ANTHROPIC_API_KEY still don't persist across Windows/Parallels PowerShell sessions (§ 4.6, unchanged, now bites three env vars instead of two).
    6. Item 20 -- RRSP/RRIF app: HLA review against MitchellNET stack, then build.
    7. Phase 3 -- Monitoring (not yet scoped). Phase 4 -- IoT (not yet scoped).

KNOWN ISSUES -- logged, not yet actioned:
    * BIS API key pasted in plaintext across multiple sessions (4-8 July 2026) and explicitly deferred again 9-10 July -- rotate. Extremely overdue, now TEN+ sessions.
    * BIS -- multimeter/power-supply/signal-generator drivers lack driver-level SCPI wire-format tests and the live-hardware test tier oscilloscope now has; unaudited. See § 4.6.
    * BIS -- request/response model nesting inconsistency across the four instrument routers, not yet audited. See § 4.6.
    * RC-Experiments -- BIS_REPO_PATH/BIS_API_KEY/ANTHROPIC_API_KEY env vars still not persisted across Windows/Parallels sessions. See § 4.6.
    * capture_validation.py's voltage-plausibility check uses a per-element Python loop instead of numpy min/max -- backlog, not urgent. See § 4.9.
    * tests/test_run_bench.py reloads run_experiment.py fresh in every test rather than caching the module -- backlog, not urgent. See § 4.9.
    * DMM cross-check readings -- deprioritized per Andrew's explicit instruction; not useful to the experiment's actual goal. Still present in run_bench(), unchanged.
    * Recipe file upload 413 -- NGINX client_max_body_size. Workaround: compress to JPEG.
    * InsanelyGoodRecipes.com import -- Andrew to verify it saved a real recipe not a listing page.
    * No UPS on server.
    * AI meal planning -- full browser functional test not yet done.
...
RESOLVED SINCE 8 JULY (no longer open):
    * Experiment 2 workflow revisit (DMM timing, side-by-side GUI, AI analysis) -- RESOLVED 9-10 July 2026. All three goals built and hardware-verified. RC-Experiments PRs #15-#22, bench-instrument-service PR #43.
    * RC-Experiments AI Analysis Windows UTF-8 encoding bug -- RESOLVED 10 July 2026, found and fixed live during hardware verification. RC-Experiments PR #20. See § 4.10.
    * RC-Experiments test-pollution bug (fake screenshot committed to results/) -- RESOLVED 10 July 2026, RC-Experiments PR #17.
    * RC-Experiments documentation gap -- RESOLVED 10 July 2026: existing markdown docs updated, formal BRD/HLA created, experiment2Guide.md written. RC-Experiments PR #21, #22.

STILL OPEN / NOT RESOLVED (do not mark these resolved without live re-verification):
    * BIS API key rotation -- still not done, now TEN+ sessions overdue, explicitly deferred again this session.
    * RC-Experiments, Experiment 1 (Bias-T) -- not started.
    * RC-Experiments -- BIS_REPO_PATH/BIS_API_KEY/ANTHROPIC_API_KEY persistence across Windows/Parallels PowerShell sessions (§ 4.6) -- unchanged, still requires manual re-set every fresh session.
