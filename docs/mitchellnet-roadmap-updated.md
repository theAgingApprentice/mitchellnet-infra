...
Last updated: 6 July 2026, late-night session (BIS oscilloscope binary-read chunking fix shipped, tested, deployed — but this did NOT resolve the underlying corruption bug; capture_dual_channel() single-request fix also shipped and deployed — also did NOT resolve it; real root cause still unknown after extensive live diagnosis. Session paused for the night with the bug still open. See § 4.7 and § 9 for full detail — read these before doing anything else next session.)

⚠️ READ THIS FIRST NEXT SESSION: The RC-Experiments bench capture bug is NOT fixed, despite two real fixes having been shipped and deployed tonight. Do not assume either shipped fix solved the problem — both were disproven live. See § 4.7 for the full diagnostic trail, the two disproven theories, the current leading (untested) theory, and the exact next steps already planned. Also see § 4.8 for a running list of every wrong assumption made tonight, so they aren't repeated.
...
Feature/Build Work
    ...
    • Item 21 (RC-Experiments, renamed from RC-Circuit) — In progress, BLOCKED on an unresolved bench-capture corruption bug. See § 9 below.
...
Found while building RC-Experiments' shared/src/bis_client.py and experiment-2-series-rc/src/run_experiment.py against the real bench_client.py and app/models/multimeter.py. All items below fixed in bench-instrument-service PRs #21–#23 (5 July 2026):
    ...
    4.7 ⚠️ ACTIVE, UNRESOLVED BUG — RC-Experiments dual-channel bench capture corruption (opened 5 July late-evening, extensively investigated 6 July late-night, still open)
...
Outstanding prerequisite: HLA review against existing MitchellNET stack (service type, NGINX routing, CI/CD pattern) before any code is written.
    ...
    • ⚠️ NEW — RC-Experiments dual-channel bench-capture corruption bug — see § 4.7 for full detail. Top priority technical item, but do the API key rotation first since it's unrelated and much cheaper to finish.
    • ⚠️ NEW — DMM cross-check voltage readings during bench captures have been consistently wrong/erratic across every attempt tonight (expected ~2.5V, actual readings scattered from -0.095V to 0.443V). Not investigated. May or may not be related to § 4.7's bug.
